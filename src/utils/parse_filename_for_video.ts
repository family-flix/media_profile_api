...
export function parse_filename_for_video(
  ...
  const special_season_with_number_regexp =
    /(^|[^a-zA-Z])([sS][pP])([0-9]{1,})($|[^a-zA-Z])/;
  if (original_name.match(special_season_with_number_regexp)) {
    // name.SP2 改成 name.SP.E2
    original_name = original_name.replace(
      special_season_with_number_regexp,
      "$1.SP.E$3.$4"
    );
  }
  original_name = original_name
    ...
    .replace(/\+{1,}/g, ".")
    .replace(
      /(^[\u4e00-\u9fa5]{1,})([0-9]{1,})(\.[a-zA-Z]{1,}[0-9a-zA-Z]{1,}$)/,
      (matched, p1, p2, p3) => {
        return (
          p1 +
          "." +
  ...
  const extra: ExtraRule[] = [
    ...
    {
      regexp:
        /_File|HDJ|RusDub|Mandarin|百度云盘下载|主演团陪看|又名|超前点播直播现场/,
      before() {
        // 存在 28(1).mkv 这种文件名
    ...
    // 来源平台
    {
      // 亚马逊、奈飞、迪士尼、爱奇艺、湖南TV
      regexp:
        /AMZN|NF|Netflix|DSNP|iQIYI|HunanTV|YYeTs|陕艺|JSTV\.{0,1}|江苏卫视\.{0,1}|[bB]站/,
    },
    // 文件后缀
    {
      key: k("type"),
    ...
    {
      regexp:
        /【{0,1}[0-9]{1,5}(\.[0-9]{1,5}){0,1}([gG]|[mM])[bB]{0,1}】{0,1}\.{0,1}$/,
    },
    {
      regexp: /[nN][oO]\.[0-9]{1,}｜/,
    },
     ...
     */
    {
      ...
      regexp:
        /本篇|完结篇|OVA([^编編篇]{1,}[编編篇]){0,1}|特典映像|番外篇|特辑篇|PV|泡面番/,
      before() {
    ...
    },
    {
      key: k("episode"),
      regexp:
        /[0-9]{4,8}\.{0,1}-{0,1}(独家){0,1}直拍(第[0-9]{1,}[期局场]){0,1}/,
    },
    {
      key: k("episode"),
    ...
    },
    {
      ...
      regexp:
        /^([123][0-9]{1,4}[-.]{0,1}){0,1}0[1-9][-.]{0,1}[0-3][0-9][期局场]{0,1}-{0,1}\.{0,1}[上下]{0,1}/,
    },
    {
      key: k("episode"),
      // 2012.10.01 12.10.01 10.01
      regexp:
        /^([123][0-9]{1,4}[-.]{0,1}){0,1}1[0-2][-.]{0,1}[0-3][0-9][期局场]{0,1}-{0,1}\.{0,1}[上下]{0,1}/,
    },
    {
      key: k("episode"),
    ...
    },
    {
      key: k("episode"),
      regexp:
        /续集|彩蛋[0-9]{0,}|花絮[0-9]{0,}|番外[0-9]{0,}|BONUS|[pP][rR][0-9]{0,}[\.$]/,
      placeholder: ".",
      priority: -1,
    },
    {
    ...
    // },
    {
      ...
      regexp: /^[a-zA-Z：]{1,}[\u4e00-\u9fa5]{1,}/,
      after(matched) {
        const episode_index = result.episode
          ? original_name.indexOf(result.episode)
          : -1;
    ...
    },
    {
      ...
      desc: "korean name",
      regexp:
        /\[{0,1}[0-9]{0,}[\uac00-\ud7a3][0-9a-zA-Z\uac00-\ud7a3，：·]{0,}[\uac00-\ud7a30-9a-zA-Z]/,
    },
    ...
    },
    {
      ...
      after(v) {
        log(
          "[3.1]need skip extracted chinese name3",
          result.episode,
          result.name,
          v
        );
        if (result.episode) {
          ...
          if (v) {
            ...
            log(
              "[3.2]need skip extracted chinese name3",
              name_index,
              episode_index
            );
            if (
    ...
    },
    {
      ...
      // 字母开头，如 Doctor异乡人
      regexp:
        /^[A-Za-z]{1,}\.{0,1}[A-Za-z0-9\u4e00-\u9fa5！：，（）～~-]{1,}[\u4e00-\u9fa5！：，（）～~-]{1,}/,
    ...
    // 影片来源
    {
      key: k("source"),
      regexp:
        /(HMAX){0,1}[wW][eE][bB](-IMAX){0,1}(-HR){0,1}([Rr][i][p]){0,1}(-{0,1}[dD][lL]){0,1}/,
    },
    {
      key: k("source"),
    ...
    // 上面没有解析出来，这里是最后的机会提取季
    {
      ...
      before() {
        ...
        log(
          "[season]filename before check skip with custom flag",
          cur_filename
        );
      ...
      const name_extra: ExtraRule[] = [
        {
          ...
          after(matched_content) {
            ...
            if (result.episode) {
              ...
              if (
                episode_index !== -1 &&
                episode_index >= original_name_index
              ) {
                return undefined;
              }
              ...
              if (
                episode_index2 !== -1 &&
                episode_index2 >= original_name_index
              ) {
                return undefined;
              }
              return {
 ...
 * @returns
 */
export function format_episode_number(
  n: string,
  ...
  if (number.match(/[上下]/)) {
    ...
    return (
      format_episode_number(remaining, options) +
      "@" +
      number.match(/[上下]/)![0]
    );
  }
  ...
  log(
    "[]format_episode_number - before ^[0-1][0-9][-.]{0,1}[0-3][0-9]",
    number
  );
...
export function maybe_other_season(episode: string) {
  if (
    episode.match(
      /花絮|彩蛋|预告|加更|番外|特别篇|集结篇|直拍|直播|超前|先导|企划|续集|Plus|BONUS|PR|NCOP|NCED|CM/
    )
  ) {
    return "其他";
  }
 ...
 * 构建一个带有首字母的电视剧名称
 */
export function build_media_name(values: {
  name: string | null;
  ...
  const name_with_pin_yin = [n, original_n]
    .filter(Boolean)
    .join(".")
    .replace(/:/, "：");
